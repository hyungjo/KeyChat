	...
	//CHANNELS에서 LEADER을 찾아 회원을 탈퇴 한다. 
		...
		public static ArrayList<ChannelsModel> findLimitAnonym() throws SQLException {
			...
			try {
				...
				rset = pstmt.executeQuery();
				while (rset.next()) {
					allList.add(new ChannelsModel(rset.getString(1), rset.getString(2), rset.getInt(3), rset.getInt(4),rset.getInt(5),rset.getInt(6),rset.getDate(7)));
				}
		...
		/*public static void insertAnonym(ChannelsDto user) throws SQLException {
			Connection con = null;
			PreparedStatement pstmt = null;
			boolean	String query = "insert into channels(limit_anonym) values(?, ?, ?, ?, ?, 'T', ?)";
			boolean String query2 = "insert into channels(limit_anonym) values(?, ?, ?, ?, ?, 'F', ?)";
			try {
				con = DBUtil.getConnection();
				pstmt = con.prepareStatement(query);
				pstmt.setString(1, user.getName());
